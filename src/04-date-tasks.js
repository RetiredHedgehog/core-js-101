...
function timeSpanToString(startDate, endDate) {
  ...
  const padZero = (n, length) => n.toString().padStart(length, '0');

  return `${padZero(hours, 2)}:${padZero(minutes, 2)}:${padZero(seconds, 2)}.${padZero(mSeconds, 3)}`;
}
...
function angleBetweenClockHands(date) {
  ...
  answ = answ > 180 ? 360 - answ : answ;

  return (Math.PI / 180) * answ;
}
